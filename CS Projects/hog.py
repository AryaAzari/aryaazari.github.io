...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    brawl_score = 3 * abs((player_score % 10) - ((opponent_score % 100) // 10))
    if brawl_score < 1:
        return 1
    else:
        return brawl_score
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    def strategy_maker(score, opponent_score):
        return n
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score < goal:
        ...
        while opponent_score < goal:
            curr = strategy(score, opponent_score)
            opponent_score += 1
            if curr != strategy(score, opponent_score):
                return False
        score += 1
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, curr_best, max_rolls = 1, 0, 1
    while num_rolls <= 10:
        avger = make_averaged(roll_dice, times_called)(num_rolls, dice)
        if avger > curr_best:
            curr_best = avger
            max_rolls = num_rolls
        elif avger == curr_best:
            max_rolls = min(num_rolls, max_rolls)
        num_rolls += 1
    return max_rolls
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    boar_score = boar_brawl(score, opponent_score) + score
